	...
	<div class="content container-fluid">
		<div class="page-header">
			<div class="row align-items-center">
				<div class="col">
					<h3 class="page-title">App Report Post List</h3>
				</div>
			</div>
		</div>
		@if(session('message'))
					...
					<div class="table-responsive">
						<table class="table table-hover custom-table datatable" id="cfi">
							...
							<tbody>
	                            ...
	                            ->select('comment.*')
	                            ->count();
	                            $postreportcount = DB::connection('mysql2')->table('reportpost')
	                            ->where('post_id','=',$val->post_id)
	                            ->where('status_id','=',1)
	                            ->select('reportpost.*')
	                            ->count();
								?>
									<tr>
										...
										<td><a onclick="viewreporteduser({{$val->post_id}})" title="View Comments" style="cursor: pointer;">{{$postreportcount}}</a></td>
										<td>{{$postlikecount}}</td>
...
<script>
	...
	function viewreporteduser($id){
		window.location.replace('{{url("/viewreporteduser")}}/'+$id);
	}
</script>
@endsection